...
# EPSILONS = [0.1, 0.5, 1.0, 5.0, 10.0, 50.0, 100.0, 500.0, 1000.0]
EPSILONS = [0.01, 0.1, 1, 10, 50, 100, 500, 1000]
PERTURBATION = 'grad_pert_'
DP = ['dp_', 'adv_cmp_', 'zcdp_', 'rdp_']
# DP = ['dp_', 'zcdp_', 'rdp_']
TYPE = ['o-', '.-', '^-', '--']
DP_LABELS = ['NC', 'AC', 'zCDP', 'RDP']
# DP_LABELS = ['NC', 'zCDP', 'RDP']
# RUNS = range(5)
RUNS = range(1)
...
def plot_advantage(result):
	...
	for dp in DP:
		...
		if args.plot == 'acc':
			...
			# plt.annotate("RDP", pretty_position(EPSILONS, y["rdp_"], 2), textcoords="offset points", xytext=(-40, 15), ha='right')
			# plt.annotate("zCDP", pretty_position(EPSILONS, y["zcdp_"], 3), textcoords="offset points", xytext=(-15, -5), ha='right')
			# plt.annotate("AC", pretty_position(EPSILONS, y["adv_cmp_"], -2), textcoords="offset points", xytext=(0, 0), ha='left')
			...
			plt.title('Shokri Membership Inference')
			# plt.annotate("RDP", pretty_position(EPSILONS, y["rdp_"], 2), textcoords="offset points", xytext=(-15, 0), ha='right')
			# plt.annotate("zCDP", pretty_position(EPSILONS, y["zcdp_"], 3), textcoords="offset points", xytext=(-5, 2), ha='right')
			# plt.annotate("AC", pretty_position(EPSILONS, y["adv_cmp_"], -2), textcoords="offset points", xytext=(0, -10), ha='left')
			# plt.annotate("NC", pretty_position(EPSILONS, y["dp_"], 4), textcoords="offset points", xytext=(0, 2), ha='right')
			...
			plt.title('Yeom Attribute Inference')
		elif args.plot == 'yeom_mi':
			...
			# plt.annotate("RDP", pretty_position(EPSILONS, y["rdp_"], 2), textcoords="offset points", xytext=(-20, 0), ha='right')
			# plt.annotate("zCDP", pretty_position(EPSILONS, y["zcdp_"], 3), textcoords="offset points", xytext=(-2, 5), ha='right')
			# plt.annotate("AC", pretty_position(EPSILONS, y["adv_cmp_"], -2), textcoords="offset points", xytext=(0, -10), ha='left')
			# plt.annotate("NC", pretty_position(EPSILONS, y["dp_"], 4), textcoords="offset points", xytext=(-17, -3), ha='right')

			plt.title('Yeom Membership Inference')
		color += 0.2
	...
	plt.annotate("RDP", pretty_position(EPSILONS, y["rdp_"], 2), textcoords="offset points", xytext=(-15, 10), ha='right')
	plt.annotate("zCDP", pretty_position(EPSILONS, y["zcdp_"], 3), textcoords="offset points", xytext=(-5, 15), ha='right')
	plt.annotate("AC", pretty_position(EPSILONS, y["adv_cmp_"], -2), textcoords="offset points", xytext=(0, 15), ha='left')
	plt.annotate("NC", pretty_position(EPSILONS, y["dp_"], 4), textcoords="offset points", xytext=(0, 5), ha='right')
...
def members_revealed_fixed_fpr(result):
	...
	# train_loss, membership, shokri_mem_confidence, per_instance_loss, per_instance_loss_all = pickle.load(open(DATA_PATH + MODEL + 'no_privacy_' + str(args.l2_ratio) + '.p', 'rb'))
...
if __name__ == '__main__':
	...
	parser.add_argument('--plot', type=str, default='yeom_mi')
	parser.add_argument('--fpr_threshold', type=float, default=0.1)
	parser.add_argument('--silent', type=int, default=1)
	parser.add_argument('--venn', type=int, default=0)
	args = parser.parse_args()
	print(vars(args))

	DATA_PATH = 'C:/Git/UM/EvaluatingDPML/results/' + str(args.dataset) + '/'
